...
FIDELITY_VARIANTS = (
    ("A63D", "A — 2× supersampling, 16 px logical type, 63-color global palette"),
    ("B63D", "B — 2× supersampling, 17 px logical type, 5 px tighter information column, 63-color global palette"),
    ("C63D", "C — 2.5× supersampling, 17 px logical type, 63-color global palette"),
)
...
def main() -> int:
    ...
    variant_sections = []
    for name, description in FIDELITY_VARIANTS:
        directory = PREVIEW / "variants" / name
        if all((directory / asset).is_file() for asset in ("dark_mode.gif", "light_mode.gif", "dark_mode.svg", "light_mode.svg")):
            variant_sections.append(f'''<section class="comparison"><div class="desktop"><h2>{description}</h2><div class="comparison-grid"><figure><figcaption>Dark GIF</figcaption><img src="variants/{name}/dark_mode.gif" alt="{description}, dark GIF"></figure><figure><figcaption>Light GIF</figcaption><img src="variants/{name}/light_mode.gif" alt="{description}, light GIF"></figure><figure><figcaption>Dark final SVG</figcaption><img src="variants/{name}/dark_mode.svg" alt="{description}, dark final SVG"></figure><figure><figcaption>Light final SVG</figcaption><img src="variants/{name}/light_mode.svg" alt="{description}, light final SVG"></figure></div></div></section>''')
    picture = '''<picture>
...
<section class="comparison"><div class="desktop"><h1>Fidelity comparison</h1><p class="note">Candidates are generated with the same animation and logical canvas. Compare final SVG/GIF sharpness at desktop width before choosing the committed renderer configuration.</p></div></section>
